...
// ফান্ডিং লিস্ট দেখার জন্য GET রাউট
app.get("/api/funds", async (req, res) => {
  try {
    const fundsCollection = db.collection("funds");
    const allFunds = await fundsCollection.find({}).sort({ createdAt: -1 }).toArray();
...
// Stripe পেমেন্ট সেশন তৈরির জন্য POST রাউট
app.post("/api/checkout_sessions", async (req, res) => {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'payment',
      success_url: `${process.env.NEXT_PUBLIC_APP_URL}/dashboard/funding?success=true`,
